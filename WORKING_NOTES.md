...
_(none — most recent draft "Standalone Training Manager" shipped 2026-05-10 as `f8b826b`. **Reminder for next time:** the draft was edited into the working tree without being committed first, so the original spec text isn't preserved in git history. Going forward, drafts should be committed to `WORKING_NOTES.md` when added so they survive.)_
