...
**✅ BOTH QUEUED DRAFTS SHIPPED 2026-07-17** (collaborative-creation usability + registration hardening round 2 — see Recently shipped). 🚨 **One blocker needs Josh: `RESEND_API_KEY` is missing from the edge-function secrets, so NO app email can send** (registration confirmations, event reminders, participant emails, trainer digest). Details in Recently shipped. Superseded queue note follows:
