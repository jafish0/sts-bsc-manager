...
- **2026-09-09 — Tracy's materials bug: `bsc_event_documents` + the `event-documents` bucket now gate on `can_admin_bsc_event` (all 3 items; 1 migration `event_documents_gate_on_can_admin_bsc_event`).**
  - **Item 1:** the three write policies were `is_admin_for_collaborative(<event's collab>)`, which is false for a trainer_admin on a standalone training (`collaborative_id = NULL`). Now `can_admin_bsc_event(event_id)` on INSERT (WITH CHECK), UPDATE (USING + WITH CHECK) and DELETE. SELECT policies untouched.
  - **Item 3 (done in the same pass):** bucket INSERT/UPDATE/DELETE were role-only (`profile_is_trainer_admin()` — any trainer could write to any event's folder). Replaced with `event_document_path_is_writable(name)`: a SECURITY DEFINER wrapper that takes the first path segment, refuses anything that is not a uuid, and calls `can_admin_bsc_event`. **Sweep result:** exactly two writers to the bucket, `EventMaterialsManager` and `EventDetail`, both `${eventId}/${uuid}.${ext}` — no writer exists that lacks the event id, so the storage policies were tightened as decided.
  - **Item 2:** new `utils/friendlyError.js` (`friendlyWriteError`, `noRowsMessage`, `isPermissionError`): RLS refusals (42501 / "row-level security" / 403) become "You don't have permission to <what>. If you think you should be able to, contact the CTAC team."; raw error → `console.error`. Applied to both materials uploaders, both materials deleters, the recategorize handler, `AddResourceModal` and the Resources delete. **Also found and fixed while there:** both delete handlers removed the storage object BEFORE the row delete, so an RLS-refused row delete (0 rows, no error) left a document pointing at a missing file — Tracy's failed delete attempts may have done exactly this had the file been hers. Row first now, `.select('id')` checked. Rule added to CLAUDE.md. Rollback-on-insert-failure preserved.
  - **Probes (rolled back), as the draft asked:** Tracy INSERT/UPDATE/DELETE on `7e4159e5…` all succeed and her storage path evaluates writable; Tracy also writes an AWARE event document (she IS on AWARE via `collaborative_trainers` — the draft's "not assigned" negative case needed the STS-BSC Demo event instead, where she is refused); Alex refused on Tracy's training and on AWARE, 0-row delete, storage path false, but writes his own STS-BSC Demo event; Josh writes both kinds; malformed path → false; anon still reads the standalone training's 2 documents (hub). Bucket still holds exactly Josh's 2 referenced objects.
  - ⬜ **Tracy: retry the real upload + delete on the 2026-09-14 training.** That live confirmation is the one that matters.

- **2026-09-09 — the app is called "CTAC App", everywhere it names itself (rename draft, done exactly as scoped).** `APP_NAME = 'CTAC App'` in `constants.js` is the single source of truth. **The dead `platformName` field:** all four program configs now read `APP_NAME` (kept, not deleted — one line to rename next time, and the shape stays for anything that later wants it per program). Surfaces: `index.html` title, Login h1 (`Admin Portal` → CTAC App, subtitle → "Center on Trauma and Children, University of Kentucky"), AdminDashboard h1 → "Admin Dashboard" (subtitle carries the app name), both Unsubscribe strings, and the three email footers (digest v3, registration-email v8, event-reminder v6 — all deployed with `verify_jwt: false` and read back). `.ics PRODID` deliberately untouched. Grep afterwards: `BSC Platform Manager` / `STS-BSC Manager` / `CTAC BSC Manager` → zero hits in `frontend/src` + `supabase/functions` other than the `constants.js:1` comment (correct). `STS-BSC` program usages: **35 before, 35 after** (the grep now finds 37 because two new comment lines in `constants.js` mention the program name to warn against renaming it). Verified in the dev browser: tab title "CTAC App", login h1/subtitle, no console errors. Auth email templates untouched (dashboard config, already say CTAC App). ⬜ Josh: the `agency_admin` sign-in check from the draft still needs a session.

- **2026-09-09 — `f297b41` items 1-3 + the 11-item role/bios/photos draft, shipped together (2 migrations: `role_model_bios_photos_profile_grants`, `staff_for_trainer_assignment_bio_photo`). Items 4-5 of `f297b41` superseded, not built.**
  - **🔴 Escalation closed (f297b41 #1).** `user_profiles`: table-wide UPDATE revoked from `anon` + `authenticated`; `authenticated` now holds column UPDATE on exactly `full_name, phone, organization, bio, photo_path, invite_accepted_at` under a self-only policy with `WITH CHECK`; a `BEFORE UPDATE` trigger refuses `role/team_id/is_active/agency_role/is_senior_leader/email/id` changes from anyone but a super_admin, the service role, or a SECURITY DEFINER RPC (so a re-added blanket GRANT at the 2026-10-30 cutover can't reopen it). **The probe from the draft, re-run verbatim against Testy's account: refused; row unchanged.** Direct self bio update still works; `team_id` write refused. Also revoked the pointless `TRUNCATE` grant from anon/authenticated on `user_profiles` and `bsc_staff`.
  - **"Remove from team" now works (f297b41 #2)** via `set_user_active(p_target, p_active)` — super_admin, trainer on the team's collab, or same-team leader; refuses self and refuses non-super_admins acting on staff. Probed as team_member (refused), agency_admin on Testy (flipped and restored), agency_admin on Tracy (refused). **Sessions are NOT revoked** on deactivate; instead `ProtectedRoute` now shows a "deactivated" screen with Sign out for any `is_active=false` profile, so the removed member loses the dashboard on their next page load. That is the decision taken; say if you want a hard revoke instead.
  - **CLAUDE.md rule added (f297b41 #3):** `.update()/.delete()` without `.select()` can't see an RLS refusal — three instances listed.
  - **A: role model locked.** `senior_leader` dropped from the role CHECK (zero users; boolean stays), `team_leader` kept, the three dead `can_*` columns dropped (zero refs in code, SQL functions, views, policies — verified). CLAUDE.md Roles section carries Josh's `trainer_admin` definition verbatim.
  - **B: trainer_admin scoping.** B1 resource tiles filter on new `myAdminProgramTypes` (AuthContext; `isSuperAdmin` checked FIRST). B2/B3 **what a trainer_admin saw before, plainly:** `collaboratives` RLS already limited both pages to assigned collabs, so Completion Tracking was effectively scoped by accident; Data Visualization was worse — the collaborative and team selectors were `isSuperAdmin`-only, so a trainer_admin with two collabs (Leah) got the alphabetically-first one **with no way to switch**, and no team filter. Now both pages scope explicitly (belt over RLS) and Data Viz shows the selectors for admin-level roles. B4: `DashboardRouter` sends `trainer_admin` to `/admin/trainer`; the Trainer Dashboard header link reads "Admin Dashboard →" for them.
  - **C: Forum.** C1 Forum card on CollaborativeDetail for ALL program types (count + 3 most recent + View all), between the hub panel and Resources. C2 `/admin/forum?collaborative=<uuid>` honoured (validated against the RLS-scoped list, falls back otherwise); dropdown keeps the URL in sync; `ForumThread` back/delete return to the same collaborative.
  - **D: bios + photos (option B).** `bsc_staff.user_id` (nullable FK, unique) + `photo_path` on both tables. **Resolution rule = `staff_directory_resolved()`** (SECURITY DEFINER; linked → account, unlinked → own; visibility mirrors bsc_staff RLS) — StaffDirectory reads it. Mapped BY ID: Ginny/Josh/Alex linked, Jessica/Stephanie null; the three directory bios copied up into the (empty) accounts, directory copies left untouched (verified 5/5 bios intact). `set_person_bio(...)` RPC (self or super_admin; linked `p_staff_id` writes the ACCOUNT — probed: Alex's account got the photo, directory stayed null, helper resolved to the account; Tracy refused on Leah's and on Jessica's row). Public bucket `trainer-photos` (2 MB, jpeg/png/webp), random filenames, client downscale to ~512px (`utils/trainerPhoto.js`), **replace = upload → RPC swap → delete old** (DELETE policy: super_admin, uploader, or any staff for an unreferenced object via `trainer_photo_is_unreferenced()`) — no cache-busting needed because paths never repeat. `PersonAvatar` = photo or initials, never a broken image. `PersonBioEditor` is the one editor: Trainer Bio card on `/admin/trainer` (D4), super_admin inline in the Trainer tab, `AddStaffModal` (which now writes bio via the RPC, has a "Linked app account" picker, and a photo control in edit mode). Trainer tab is read-only for trainers with a link to the dashboard. `training_hub_trainers` + `staff_for_trainer_assignment` return `photo_path` (and bio). CLAUDE.md's "don't widen it" line rewritten as the deliberate reversal.
  - **E1** BSC Events → shared `components/CollapsibleCard.jsx` (extracted from TrainerDashboard; new `storageKey` + `actions` props), collapsed by default, header shows "N sessions", state in localStorage keyed `bsc_events_open:<userId>:<collabId>` (try/catch). **E2** `collaboratives.hub_display_name`; `hub_lookup` returns `coalesce(hub_display_name, name)` as `name` (+ `collaborative_name`); "Rename hub" in the Participant Hub panel (super_admin). Verified: demo hub title changed on the public page, AWARE's unchanged, then reset to null.
  - **Verification honesty.** SQL-level: everything above. Browser: public pages only (demo TIPE hub title, Tracy's training hub renders initials + no email string in the DOM). ⬜ **Admin click-through still needs a session** — the dev browser has no login and the two test accounts' passwords are (correctly) not recorded. Josh: sign in as Leah/Alex/Ginny per the draft's matrix (Ginny must still see everything), open two Forum cards, upload a photo as Tracy and one for Jessica as super_admin, remove a member from the demo team.
...
- **2026-09-08 — 🔴 Resend-invite no longer destroys trainer assignments; the "expired link" page is actionable; plus a pre-existing infinite-loop fix on that same page. `invite-team-leader` v11 deployed (`verify_jwt: false` passed explicitly; deployed source read back and matches the repo).**
  ...
  - **Item 2 (copy):** `/set-password` with a missing/invalid/consumed token now says "This link can't be used — sign-in links work once and expire…", offers **Sign in** and **Forgot password** buttons, and honestly does not claim to know expired vs used (Supabase reports both the same way). `/login?reset=1` deep-links straight into the forgot-password modal.
  - **🐛 Pre-existing bug found while verifying, fixed:** `AuthRedirectHandler` navigated to `/set-password`+hash on every location change, so whenever Supabase could NOT consume the token (expired/used link — the exact case this draft is about) it looped forever: "Maximum update depth exceeded" on the very page meant to tell the person what to do. Now a no-op when already on `/set-password`. Verified in-browser: junk invite/recovery hashes render the new copy with zero console errors, and a `/login#…type=recovery` link still redirects to `/set-password` with the hash intact.
  - **Verified:** esbuild parse; deployed source == repo; live probes (no auth → 401, anon bearer → 401); both public pages in-browser. ⬜ **Not verifiable without a super_admin session:** the actual `already_accepted` / `has_assignments` refusals against a live account (the assertion that matters). Tracy's rows are intact right now (1 collab + lead on Sept 14); **Josh: the guard is deployed, so Resend on her is now safe — it will simply refuse.**
  - ⚠️ **Data observation for Josh:** `user_profiles` now holds a **second Tracy account**, `tracy.clemans@uky.edu` (trainer_admin, accepted, 1 collaborative assigned, no training assignments) alongside `taclem1@uky.edu`. Not touched; worth deciding which one she uses so she isn't split across two logins.

- **2026-09-01 — `event_trainers`: assignable trainers for standalone trainings (all 6 items + Tracy assigned). 1 migration. ✅ Tracy Clemans is now LEAD trainer on the Sept 14 training and Josh is removed (Josh stays `created_by`, so his ownership access is intact). The public hub shows "Tracy Clemans, PsyD, CSW" — verified in-browser, no email anywhere on the page.**
  - **Schema:** `event_trainers` mirrors `collaborative_trainers` (unique `(bsc_event_id, user_id)`, one-lead partial index, explicit grants to `authenticated`/`service_role` and **nothing to anon**). RLS: admin-level SELECT (the list page's Trainer column must show who runs trainings you can't manage); INSERT/UPDATE/DELETE gated on `can_admin_bsc_event`. `can_admin_bsc_event`'s standalone branch now also admits assigned trainers; `created_by` clause kept; collaborative branch untouched.
  - **Invariant enforced in the DB, not just the UI:** backfill seeded the creator as lead on both existing trainings; an AFTER INSERT trigger seeds the creator on every new standalone training regardless of client; a BEFORE DELETE trigger refuses to remove the last trainer (`last_trainer`) and an AFTER DELETE trigger auto-promotes the next by `sort_order` when the lead is removed — both skip during the event's own ON DELETE CASCADE. `set_event_lead_trainer()` switches the lead atomically (a naive second UPDATE trips the one-lead index). All probed in a rolled-back transaction.
  - **Two SECURITY DEFINER RPCs, and why:** `staff_for_trainer_assignment()` — `user_profiles` RLS lets a trainer_admin read only their own + team profiles, so the picker and every name lookup (TrainingsAdmin column, modal, Trainer Dashboard) would otherwise show a trainer nobody but themselves; admin-level callers, staff roles only, includes email so similar names are distinguishable (admin-only surface). `training_hub_trainers(p_hub_token)` — name + bio + is_lead ONLY for hub-enabled standalone trainings; **closes the privacy leak** where the hub printed a staff email whenever a name was missing.
  - **Bio finding (draft asked me to say which way):** `user_profiles` UPDATE RLS is **self-only** — there is no super_admin update policy either. Per the draft I did **not** widen it: the modal's bio editor works only for yourself; other trainers show a has-bio / no-bio state with "only they can write it, from this tab." **Tracy has no bio** → her hub card is name-only until she writes one (she can, once she opens the training — she's assigned so she can manage it).
  - **UI:** modal Trainer tab (edit mode) lists trainers with LEAD badge, Make lead, Remove (disabled + tooltip on the last one), add-picker with name — email; create mode explains the creator auto-becomes lead. TrainingsAdmin Trainer column = lead "+N", `can_manage` includes assigned trainers; EventDetail `canManage` likewise. TrainingHub renders a list, pluralised heading, LEAD badge when >1. **Trainer Dashboard** gained "My Standalone Trainings" (scoped by `event_trainers`, so an assigned trainer sees trainings someone else set up — Tracy's case) and standalone trainings in the 4-week upcoming window; a trainer with zero collaboratives no longer hits the early return before seeing them.
  - **Verified:** backfill both trainings; hub RPC (Josh→Tracy on Sept 14; Aug 7 returns nothing because its hub is off); triggers + one-lead index in a rolled-back probe; in-browser single trainer and temporary two-trainer render (Tracy first, LEAD, plural heading, no `@` in the page); collaborative events untouched. ⬜ Not click-verified (admin-gated): the modal's assign/remove/lead controls, Tracy's Trainer Dashboard, TrainingsAdmin column — **Tracy's own login is the ideal test.**
  - **✅ 2026-09-08, the `last_trainer` trigger fired in production and did its job.** Josh, cleaning up a duplicate Tracy account he'd created by inviting her Link Blue alias, deleted the right one (`tracy.clemans@uky.edu`) and then attempted the keeper. Postgres refused with `last_trainer` **five times** (19:43:20 → 19:45:10 UTC), because she is LEAD and sole trainer on the **2026-09-14** training. Her `event_trainers` + `collaborative_trainers` rows are intact, verified after. So the guard was exercised in anger against real data six days out from the training, which is stronger evidence than the rolled-back probe. One UX caveat outside our control: the Supabase dashboard swallows the reason and shows only "Database error deleting user", so any future delete refusal will look generic.
  - Note for Josh: `set_event_lead_trainer` correctly refuses when there's no `auth.uid()` — a SQL-console call fails with `not_authorized`; that's the guard working, not a bug.

- **2026-09-01 — all 4 of Josh's same-day Feedback-widget items shipped (discussed with Josh first; his calls recorded inline). All four `app_feedback` rows marked `incorporated` with notes. `invite-team-leader` v10 deployed with `verify_jwt: false` passed explicitly.**
  - **Standalone training hub opens permanently (Josh: drop all three gates).** `/training/:token` no longer has the pre-start window, the post-end closure, or the per-device signed-in flag — link = access, matching the TIPE hub posture. The page now gates on `hub_enabled` instead (previously the toggle only affected the sign-in redirect; with a permanently-open hub it has to gate the page itself). `SessionMaterials` (collaborative flow) still uses the sessionStorage flag, so sign-in keeps setting it — deliberately out of scope. **Verified in-browser on the real Sept 14 training** (the exact one Josh commented from, future-dated = the exact gate he hit): renders fresh-context with materials downloadable, no gates, no console errors.
  - **Role/Job Title required at session sign-in** (was optional): required attribute + submit guard + asterisk, same treatment as Agency. Client-side only, matching how Agency is enforced.
  - **Parking-lot participant intake is opt-in per event** — new `bsc_events.parking_lot_enabled` DEFAULT false ("we don't want people leaving comments if the trainer isn't going to read them"). Checkbox lives on EventDetail's parking-lot section; it gates the **participant-facing intake only** (TeamDashboard card + hub question box, per my recommendation Josh took) — the admin tracker stays available always. `hub_post_parking_lot` refuses server-side (`parking_lot_disabled`) and `hub_lookup` exposes the flag. **Verified:** RPC gate both ways in SQL (probe deleted, flags restored), hub card hidden/shown in-browser on the demo hub. ⚠️ **Default false applies to existing events too — the AWARE sessions' hub question box stays hidden until Josh ticks each session on.**
  - **"Add CTAC Staff" on the Admin Dashboard (super_admin only).** New `InviteStaffModal` → extended `invite-team-leader`: staff roles `super_admin` / `trainer_admin` (super_admin callers only, no team), `collaborative_ids[]` for trainer assignments via `collaborative_trainers` (validated up front, whole invite rolled back if assignment fails, `is_coordinator` never set from here), 409 → confirm-and-resend path. **No password is emailed, deliberately** — Josh's ask was "email with username and password", shipped instead as the existing invite-link → `/set-password` flow (same outcome, no credential in an inbox); flagged here so the deviation is visible. Deployed v10; deployed source read back and matched; live probes: no auth → 401, anon-key bearer → 401. ⬜ The happy path (real invite email → set password → trainer scoping) needs Josh — and doubles as the long-wanted test-account creation: inviting a demo trainer_admin would finally give Claude Code a click-through account (record it in CLAUDE.md, never the password).
  - Also: `.claude/launch.json` gained a `frontend-dev-alt` config on port 5199 (another session's dev server was holding 5173).

- **2026-08-27 `f4c4845` — TIPE Learning Collaborative Roster replaces Teams/Team Rosters; Participant Hub restricted to TIPE (follow-on draft, all items incl. the optional export).**
  - **Item 1b shipped first:** the hub panel renders only on `tipe_lc`, and `hub_collab_for_token` — the one choke point every hub RPC (reads AND writes) resolves tokens through — now requires `program_type = 'tipe_lc'`, so a token minted on a non-TIPE collab is refused server-side. Verified both directions with a rolled-back probe (sts_bsc row with `hub_enabled=true` + token → `hub_lookup` returns NULL; the live TIPE token still resolves). Data was confirmed clean first: only the two TIPE collabs carry tokens.
  - Teams + Team Rosters hidden for `tipe_lc` only, **UI-only** (no team rows or `demo_roster` deleted — reversible; the TIPE LC Demo's 6 demo teams disappearing is the accepted consequence). TIC/STS untouched.
  - **New `LearningCollaborativeRoster`** (registrations-sourced, cancelled excluded, deduped on normalized email): Name/District/School/Position/Email from `responses` with missing keys tolerated; switchable grouping (District → School+Name secondary / School / Name) with per-group + total counts; the early-registrant data-quality caveat stated in the UI, answers shown verbatim. **Live sign-in status:** session selector defaulting to the current session (hub rule), per-row signed-in (ET time)/signed-out/not-signed-in, `N of M signed in` summary, Realtime + 30s fallback. Matching is `=` on lowercased+trimmed email — never `ilike`. **Walk-ins get their own "signed in but not registered" group** instead of being dropped. Read-only.
  - The optional Excel export was cheap: a small generic `downloadRowsAsExcel()` added to `exportAttendance.js` (same conventions — frozen header, widths, header-when-empty) rather than forcing roster columns through the attendance-shaped builder.
  - **Verified:** RPC gate probe (rolled back); AWARE shape matches the draft — now **44** non-cancelled (up from the draft's 42), all-distinct emails so the dedupe loses nobody, exactly **3 blank Schools** (the old combined label), 0 blank Districts. ⬜ Admin-gated click-through (roster render, live status during a session, export) is Josh's — still no test account.
...
### 2026-08-26: TIPE Collaborative Detail — replace Teams/Team Rosters with a Learning Collaborative Roster — ✅ SHIPPED 2026-08-27 (`f4c4845` + 1 migration; incl. item 1b hub restriction and the optional export; Josh's click-through pending) — spec kept for reference
...
### 2026-08-27: Assignable trainers for standalone trainings (`event_trainers`) — ✅ SHIPPED 2026-09-01 (1 migration; Tracy assigned as lead on Sept 14, Josh removed; admin-gated click-through pending — Tracy's login is the test) — spec kept for reference
...
### 2026-09-08: 🔴 Resend-invite silently destroys trainer assignments + "expired link" is misleading — ✅ SHIPPED 2026-09-08 (invite-team-leader v11 + UI; the live refusal against a real account still needs a super_admin click — it is now safe to try on Tracy) — spec kept for reference
...
### 2026-09-09: 🔴 `user_profiles` privilege escalation + two silent-no-op writes, and trainer bios/photos (5 items) — ✅ ITEMS 1-3 SHIPPED 2026-09-09 (with the draft below); items 4-5 SUPERSEDED by section D below, not built — spec kept for reference
...
### 2026-09-09: Role model locked, trainer_admin scoping, Forum card, bios + photos, 2 small UI items (11 items) — ✅ SHIPPED 2026-09-09 (2 migrations; SQL-probed; admin click-through matrix is Josh's — see Recently shipped) — spec kept for reference
...
### 2026-09-09: 🔴 Trainers cannot add or delete materials on their own standalone trainings (3 items) — ✅ SHIPPED 2026-09-09 (all 3 items, 1 migration, probed as Tracy/Alex/Josh; Tracy's live retry pending) — spec kept for reference
