...
- **2026-06-10 (data op — Cowork + Josh)** — TIPE resource library **LOADED** (supersedes the `a2c0fb8` "not yet run" note). Josh ran `scripts/seed_tipe_resources.py --commit`: **249 files** → `resources` bucket (314 objects total), **26 categories**, 16 multi-category files correctly multi-tagged. **Two fixes mid-run:** (1) broadened the `resources_resource_type_check` CHECK constraint — it only allowed `pdf/docx/pptx/doc/youtube/link`, but the script inserted `resource_type='file'` and the zip has html/xlsx/jpg/webp; now allows those + common types; (2) patched `seed_tipe_resources.py` to set `resource_type` from the real file extension instead of literal `'file'` (**uncommitted — Josh to `git add`**). **Videos category** finished via SQL: 24 YouTube embeds + 5 non-YouTube links + the source PDF (script's pypdf embed step didn't run). ⚠️ **Script is NOT idempotent** — resources-row insert has no skip-existing (docstring wrongly claims it does); re-running duplicates all 249 rows. See the "Fix seed_tipe_resources.py idempotency" task.

- **2026-06-10 `a2c0fb8`** — ⚠️ TIPE resource bulk-load **script delivered, NOT yet run** (draft at `1549808`). `scripts/seed_tipe_resources.py` loads the TIPE library from `Resources/TIPE/Basecamp Download.zip`. **Blocked on Josh:** uploading to the private `resources` bucket requires `is_super_admin()` (storage RLS) — Claude Code only has the anon key and can't handle the service_role key, so the actual upload is Josh's manual run (like the Vault key was). Dry-run validated end-to-end with no credentials: 332 files → 54 excluded (the 9 session/call folders) → **249 unique by SHA-256**, 16 multi-category, **0 untagged**, **26 categories** (24 topic folders + Trauma-Informed Schools + Videos), 2 Wisconsin `.docx` twins dropped for their `.pdf`. To run: `$env:SUPABASE_SERVICE_ROLE_KEY="…"; py scripts/seed_tipe_resources.py --commit`. One deferred piece: the Videos PDF's embedded YouTube/link rows need `pip install pypdf` (Videos category still gets the source PDF; links addable on a rerun).
...
_One ready Claude Code item below — **CSV export for the feedback dashboard**. Then two non-build items: **(A)** the TIPE resource bulk-load (script ready, ⛔ needs Josh to run with the service key), and **(B)** the ⛔ BLOCKED data-cleaning stage (skip it). (Shipped 2026-06-10: config guardrails `37d5bd1`, View-as `774416a`, session materials `14ad573`; earlier — demo-data seed `2624ed2`, feedback triage `00f15ce`, ProQOL burnout-only `ae1fd09`, CEU course-correction `9b01b22`, feedback widget `a52463d`.)_

### 2026-06-10 — Add CSV export to the feedback triage dashboard

> Small add. Josh confirmed the existing auth-gated feedback widget + `/admin/feedback` dashboard cover the testers (they log in as super_admin); the one piece missing from the reference design is CSV export for synthesis. (Decided NOT to build the anonymous/no-login roster model.) Commit this draft first.

In `frontend/src/pages/FeedbackAdmin.jsx`, add an "Export CSV" button at the top (page is already `isSuperAdmin`-gated). Export the feedback rows with ALL fields — `id, created_at, user_email, user_role, category, severity, route, page_label, program_type, collaborative_id, message, status, admin_notes, resolved_at` — so the triage decision (`status`/`admin_notes`) is paired with each row for synthesis. Export the **currently-filtered** set (respect the active status/category/severity/program/submitter filters + search) so it matches what's on screen. Generate client-side and trigger a download, with proper escaping of commas/quotes/newlines in `message` and `admin_notes`. No backend change.

**Verify:** Export downloads a CSV with all columns; commas/quotes/newlines in messages don't break columns; the filtered export matches the on-screen rows.

### 2026-06-10 — Bulk-load TIPE LC resource library ⏸ SCRIPT READY — needs Josh to run with the service key

> The script is written, committed (`a2c0fb8`, `scripts/seed_tipe_resources.py`), and dry-run validated. It is **not run yet**: uploading to the private `resources` bucket requires `is_super_admin()` (storage RLS), and Claude Code only has the anon key + must not handle the service_role key. So this last step is Josh's, like the Vault key setup was.
>
> **To run it** (PowerShell, from repo root): `$env:SUPABASE_SERVICE_ROLE_KEY = "<service role key from Supabase → Settings → API>"; py scripts/seed_tipe_resources.py --commit`. (Run it without `--commit` first to re-print the plan — no key needed.) The script is idempotent: categories upsert on `(program_type, category_key)`, files are keyed by content hash so re-runs overwrite the same object, and resources rows key off `file_path` (= hash).
>
> **Dry-run already confirmed:** 332 files → 54 excluded (the 9 session/call folders) → **249 unique by SHA-256**, 16 multi-category, **0 untagged**, **26 categories** (24 topic folders + Trauma-Informed Schools + Videos), Wisconsin `.docx` dropped for its `.pdf` twin, loose Attachments mapped per the draft.
>
> **One deferred piece — the Videos embedded links.** Parsing `Video Links for TIPE Trainings_April 2025.pdf` into `youtube`/`link` resource rows needs `pypdf` (not installed). The Videos category gets the source PDF regardless; to add the inline clips, `py -m pip install pypdf` and extend the script's Videos step (the raw PDF text has some garbled links — validate each 11-char YouTube id, skip/flag the rest). Lower priority than the file load.

<details><summary>Original full draft spec (for reference)</summary>

Loads the TIPE library from `Resources/TIPE/Basecamp Download.zip`. Topic folders → the TIPE resource library; session/call folders are EXCLUDED here (handled by the session-materials feature). One TIPE category per topic folder (granular, as-is) PLUS **Trauma-Informed Schools**, **School Intervention Project** (its own), and **Videos**. EXCLUDE: Learning Session 1/2/3, both Learning Session 4 folders, Learning Calls 1/2/3, "Session 5 Secondary Traumatic Stress…". Dedupe by content hash, store once in the `resources` bucket, one row per unique file with a `tags` array of EVERY category it appeared in (Resources page filters category-mode by `tags.includes(category)`). Loose `Attachments/` mapped: National Safe & Supportive Schools + Wisconsin Trauma-Sensitive Schools → Trauma-Informed Schools; "15 Sites & Apps for SEL" → Social Emotional Learning; "Let's Talk" → Community Engagement; "MindUP Activities" → Mindfulness in Schools; "Mindfulness Scripts & Videos for Kids" dedupes against the existing library copy. Videos: keep the PDF as a download, create `youtube` rows per validated link and `link` rows for non-YouTube (Vimeo, cctassifilms.org, clark.wa.gov ACES, ctac.uky.edu/meditations).
