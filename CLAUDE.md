...
- **⚠️ Never show a raw RLS refusal to the user.** `new row violates row-level security policy for table …` is what Tracy saw (2026-09-09) trying to upload a handout to her own training. Route write failures through `friendlyWriteError(err, 'what they were doing')` / `noRowsMessage(...)` in `utils/friendlyError.js` (human message, raw error to `console.error`). And **delete the row before the storage object** — the reverse order plus an RLS-filtered 0-row delete leaves a document pointing at a file that no longer exists.
- **`bsc_event_documents` + the `event-documents` bucket are gated on `can_admin_bsc_event(event_id)`** (2026-09-09) — super_admin, the creator, or an assigned trainer for standalone trainings; `is_admin_for_collaborative` for collaborative events. Storage paths must be `<event_id>/<uuid>.<ext>`; the bucket's INSERT/UPDATE/DELETE policies derive the event from the first path segment via `event_document_path_is_writable(name)`. Do not gate either on `is_admin_for_collaborative` alone — standalone trainings have `collaborative_id = NULL`, which is why trainers could not touch their own materials for a week.
